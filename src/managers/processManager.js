const ProcessManager = (function () {
  ...
  return {
    ...
    generatePalette() {
      ...
      if (DOMManager.spaceSelect.selected() === 'Show All') {
        const iMax = DOMManager.colorCount.value() > iLimit ? iLimit : DOMManager.colorCount.value();
        ...
        for (let i = 0; i < iMax; i++) {
          ...
          drawColors(sRGB.mix(start, end, t), posX, posY);

          posY += squareSize + 5;
          drawColors(LinearRGB.mix(start, end, t), posX, posY);

          posY += squareSize + 5;
          drawColors(CIEXYZ.mix(start, end, t), posX, posY);

          posY += squareSize + 5;
          drawColors(OkLab.mix(start, end, t), posX, posY);
        }
      } else {
        // console.log(iLimit);
